...
Tomato Tap 保留这些策略的本地可见性和可审计性。若你需要组织、租户、账单结算和分布式控制面，New API、Sub2API 等多租户网关通常更合适。
...
| 需要给其他用户使用 | 启用下游认证，并阅读 [SECURITY.md](SECURITY.md) |
...
| 概念 | 管理内容 | 不包含什么 |
|---|---|---|
| 供应商 / Channel | 地址、协议、认证方式、RPM、并发上限 | 不保存明文 Key 到公共配置 |
...
服务默认监听 `127.0.0.1` 并信任本机下游。不要把可信模式直接暴露到不受信任的网络。二级分发前请配置下游认证并阅读 [SECURITY.md](SECURITY.md)。
